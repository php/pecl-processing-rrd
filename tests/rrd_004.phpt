...
--FILE--
<?php
$rrdFile = dirname(__FILE__) . "/updateTest.rrd";
...
$command = "rrdtool graph ".dirname(__FILE__)."/updateTest.png "
...
echo "exporting updateTest.png via exec\n";
exec($command);

$command = "rrdtool fetch $rrdFile AVERAGE --start 920804400 --end 920809200";
$output = array();
exec($command, $output);
$originalFetch = file(dirname(__FILE__)."/testData/updateFetch.txt", FILE_IGNORE_NEW_LINES);
...
exporting updateTest.png via exec
